...
Описание работы:
 ...
 Программа загружает из XML-файла ГГГГММДДџED807_full.xml (описан в документации УФЭБС ЦБ РФ© данные о клиентах МЦОИ
...
----------------------------------------------------------
15.12.2018
Реализован экспорт в bnkseek.dbf (по F9). Если AutoSave=YES в ini-файле, то экспорт в bnkseek.dbf происходит автоматически.
...
----------------------------------------------------------
22.01.2019
Тип экспортируемого файла изменен на FOX2X (кодовая страница 866). Добавлено заполнение поля NAMEN.
Теперь bnkseek.dbf корректно открывается в старом справочнике (MSDOS).

----------------------------------------------------------
20.09.2019
Исправлена досадная ошибка (когда рестриктов по счету больше 1). Добавлена ещё одна таблица (accr807ГГГГММДД®dbf)
для хранения вышеуказанных рестриктов. Подправлен вывод в текст. файл из просмотра и ускорена процедура
заполнения таблиц. Соответственно прошлые даты будут показываться как отсутствующие (кнопка "Импорт"- 
оранжевая). Нужно будет перезакачать эти даты.
----------------------------------------------------------
